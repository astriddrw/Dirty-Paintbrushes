...
- Instrument Serif italic is a rare, deliberate signal — the wordmark and the homepage hero, nowhere else — not a default headline treatment; Archivo carries page-level headings, Lora carries article titles, Roboto anything that has to be read fast.
...
**Headline Font:** Archivo (with Arial, sans-serif fallback) — page-level `<h1>` titles and section headings.
...
**Character:** Instrument Serif's italic sets the voice — editorial, a little literary, never neutral — but it was showing up on nearly every headline on the site (wordmark, hero, section headings, page titles, article titles), which read as templated rather than considered. Lora first absorbed every non-hero headline, but at `text-4xl`/`text-5xl` a second serif still read too close in register to the Instrument Serif hero directly above it on most pages — two serifs of similar weight sharing a page looked like an unresolved choice rather than a considered one. Archivo now carries page-level headings instead: a heavy grotesque with genuinely blocky proportions, distinct from both serifs by construction rather than degree. Lora stays scoped to article titles, where its smaller scale (`text-base`/`text-lg`) never reads as competing with the hero. Roboto stays completely out of the way for anything that has to be scanned (filters, admin tables).

### Hierarchy
- **Display** (400, `clamp(3rem, 6vw, 4.5rem)`, italic, leading-tight): the homepage hero headline only.
- **Headline** (600, Archivo, `text-4xl` → `text-5xl`, tracking-tight): every page's `<h1>` (Sources, About, Saved, Feed's "Latest News", the gated Cases page) via the shared `FadeInHeading` component, plus the homepage's "Latest Intelligence" section heading (`text-2xl`/`text-3xl`, same face). Article detail `<h1>` and the footer wordmark still use Instrument Serif, not italic, and have **not** been moved off it yet (see Named Rules).
...
**The Serif Scarcity Rule.** Instrument Serif was overused — leaning on the same distinctive italic face for every headline is what made the site read as templated rather than authored. As of 2026-08-14 it's scoped to the wordmark and homepage hero only. Everything else was first moved to Lora, then page-level headings (every `FadeInHeading` title plus the "Latest Intelligence" heading) moved a second time to Archivo, a sans-serif, once a same-page serif-next-to-serif pairing turned out to have its own version of the same problem — two similar faces reading as one unresolved choice. Article-row titles stay on Lora, where the smaller scale keeps it from competing with the hero. Article detail `<h1>` and the footer wordmark still use Instrument Serif and have **not** been moved yet — extend this rule to them before reaching for Instrument Serif on anything new, rather than treating it as the default headline face again.
...
- **Do** use Archivo for page-level headings, Lora for article titles, and reserve Instrument Serif for the wordmark and hero — see the Serif Scarcity Rule before adding a new Instrument Serif headline anywhere else.
